...
Por último, se ha realizado a partir de los datos obtenidos del entrevistado sobre si era o no el mayor sustentador del hogar una imputación del nivel socioeconómico del hogar de residencia del entrevistado, con las siguiente distribución resultante:
...
as.datatable_widget(tab)
```
Las preguntas del final del cuestionario pretendían estimar el volumen de ingresos d ela unidad familiar donde reside o de dónde depende el individuo entrevistado. Así, siguiendo los cálculos de la AIMC utilizados en el EGM español, los resultados se pueden ver en este gráfico.
...
highchart() %>%
    ...
    hc_chart(type = 'pie', polar = FALSE, inverted = FALSE) %>% 
    hc_xAxis(categories = df$name) %>%
    hc_add_series(df,name = "Status", showInLegend = TRUE, innerSize='75%' )%>%
    hc_credits(enabled=TRUE, text='InvestigaOnline.com', href ='https://www.investigaonline.com') %>%
    hc_exporting(enabled=TRUE)
```
